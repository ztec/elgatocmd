"""Preset buttons backed by slots stored on each physical light."""
...
async def async_setup_entry(
    ...
    """Create two preset buttons for each retained light."""
    ...
    def add_device(device_id: str) -> None:
        ...
        added.add(device_id)
        async_add_entities(
            [ElgatoPresetButton(hub, device_id, preset) for preset in (1, 2)]
        )
...
class ElgatoPresetButton(ButtonEntity):
    """A stateless control that recalls one hardware preset."""
    ...
    def __init__(self, hub: BridgeHub, device_id: str, preset: int) -> None:
        ...
        self._attr_unique_id = f"{device_id}_preset_{preset}_button"
        self._attr_translation_key = f"preset_{preset}"
        self._attr_icon = f"mdi:numeric-{preset}-box-outline"
        self._apply_model()

    @property
    def device_info(self) -> DeviceInfo:
        """Attach the button to the physical light device."""
    ...
    async def async_press(self) -> None:
        ...
        await self._hub.async_apply_preset(self._device_id, self._preset)
