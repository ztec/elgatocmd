...
class PendingCommand:
    """One command awaiting its authoritative daemon response."""
...
class BridgeHub:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self._device_listeners: dict[str, set[Callable[[Context], None]]] = {}
    ...
    def subscribe_device(
        self, device_id: str, listener: Callable[[Context], None]
    ) -> Callable[[], None]:
    ...
    async def async_command_result(
        ...
        expected_device_id = pending.device_id
        ...
        if msg.get("light") is not None:
            ...
            if device["id"] != expected_device_id:
                ...
                raise err
            self._put_device(device, pending.context)
            await self._async_save()
        elif msg["success"]:
    ...
    async def async_command(
        ...
        *,
        ...
        await self._async_command(device_id, {"update": update}, context=context)

    async def async_apply_preset(
        self, device_id: str, preset: int, *, context: Context | None = None
    ) -> None:
        ...
        await self._async_command(device_id, {"preset": preset}, context=context)
    ...
    async def _async_command(
        ...
        *,
        ...
        session.pending[request_id] = PendingCommand(
            device_id=device_id,
    ...
    def _put_device(
        self, device: dict[str, Any], context: Context | None = None
    ) -> None:
        device_id = device["id"]
        is_new = device_id not in self._devices
        if not is_new and self._devices[device_id] == device:
            return
        self._devices[device_id] = device
        update_context = context if context is not None else Context()
        for listener in tuple(self._device_listeners.get(device_id, ())):
            listener(update_context)
        if is_new:
    ...
    def _close_session(self, session: BridgeSession, reason: str) -> None:
        ...
        for pending in tuple(session.pending.values()):
            if not pending.future.done():
                pending.future.set_exception(HomeAssistantError(reason))
